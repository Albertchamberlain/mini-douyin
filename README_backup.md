...
7. - [ ] 令牌桶算法能够在限制数据的平均传输速率的同时，保证服务端不被打垮，还允许某种程度的突发传输，使用了go自带的限流模块time/rate,也可以看看一些开源实现
8. - [ ] 点赞消息异步化处理  选型消息队列,Maybe NSQ,Maybe Kafka
9. - [ ] 缓存  一切涉及查询重要数据的地方都可以进行缓存，空间换时间 优化拉满,内存不要钱
...
3. - [ ] 尽可能的并行化
4. - [ ] May be it's can include Neo4j or Nebula
